...
class Citador {
	...
	log(message, method = 'log') {
		if (console[method]) {
			...
			console.log(`Logging method '${method}' not supported`);
		}
	}
	start() {
					...
					if ($(this).find('.citar-btn').length == 0) {
						...
						.click(function () {
							...
							this.createQuote = function () {
								...
								channel = self.getOwnerInstance(elemento, {
										include: ["Channel"]
									}),
								canEmbed = channel.state.channel.isPrivate() || channel.can(0x4800, {
	...
	attachParser() {
		...
		this.handleKeypress = function (e) {
			...
			try {
				if (isQuote == true) {
					if (e.shiftKey || $('.channel-textarea-autocomplete-inner').length >= 1)
						return;
					...
					quoteMsg.find('pre').each(function () {
						$(this).html(`${$(this).find('code').attr('class').split(' ')[1] || ""}\n${$(this).find('code').text()}`)
					});
					...
					$('.messages .message-group').hasClass('compact') ? quoteMsg.find('.message-content').each(function () {
						text += `${$(this).clone().end().text()}\n`;
					}) : quoteMsg.find('.markup').each(function () {
						text += `${$(this).clone().end().text()}\n`;
					});
					...
					if ($('.quote-msg').find('.attachment').length >= 1) {
						for (var i = 0; i < $('.quote-msg').find('.attachment').length; i++) {
							...
							attachNum = i + 1,
							emoji = '📁';
							if (/(.apk|.appx|.pkg|.deb)$/.test(value)) {
								emoji = '📦'
							}
							if (/(.jpg|.png|.gif)$/.test(value)) {
								emoji = '🖼'
							}
							if (/(.zip|.rar|.tar.gz)$/.test(value)) {
								emoji = '📚'
							}
							if (/(.txt)$/.test(value)) {
								emoji = '📄'
							...
							data.embed.fields.push({
								name: `${stringLocal.attachment} #${attachNum}`,
								value: `${emoji} [${value}](${link})`
							});
						}
					}
					...
					$(this).val("").focus()[0].dispatchEvent(new Event('input', {
							bubbles: true
						}));
	...
	getDescription() {
		return "Quote messages using Discord embeds";
	}
	getVersion() {
		return "1.5.5";
	}
	getAuthor() {
		return "Nirewen (Alterations by TheBanHammer)";
	}
	getSettingsPanel() {
		return "";
	}
	...
	onSwitch() {
		...
		if (isQuote == true) {
			var elemento = document.querySelector(".messages"),
			channel = this.getOwnerInstance(elemento, {
					include: ["Channel"]
				}),
			canEmbed = channel.state.channel.isPrivate() || channel.can(0x4800, {
					channelId: channel.state.channel.id
